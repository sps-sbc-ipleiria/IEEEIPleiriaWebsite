...
import SPSLogo from '../../assets/branches/APS.jpeg';

export default function SectionAPS() {
  const { lang } = useLanguage();
  const t = translations[lang].sectionAPS;

  return (
    <section className="text-[#009ca6] py-20 px-6">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-[#c5ebec] border-2 border-[#009ca6]  p-6 rounded-lg mb-12 shadow">
        <motion.h3
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="text-2xl font-semibold mb-4 text-[#009ca6]"
        >
            {t.about.title}
        </motion.h3>
        ...
        <motion.p
          ...
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="mt-16 text-center text-lg font-medium text-[#009ca6]"
        >
          <Link
            to="/events"
            className="text-[#009ca6] underline hover:text-[#003366] transition"
          >
            {t.seeEvents}
          </Link>
        </motion.p>
